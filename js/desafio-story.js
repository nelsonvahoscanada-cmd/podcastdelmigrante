/*
  desafio-story.js — Plantilla de historia de Desafío 100 Empresas
  ======================================================================
  Una sola plantilla para las tres (y futuras) historias, leída desde
  js/desafio-stories.js por slug — mismo patrón que article.js/
  column.js/colaborador.js.
========================================================================= */
...
(function () {
  ...
  function findStory(slug) {
    return DESAFIO_STORIES.find((s) => s.slug === slug) || null;
  }
  ...
  function heroMediaHtml(story) {
    const alt = story.image && story.image.alt ? story.image.alt : story.pageTitle;
    return `<div class="article-hero-media" style="background-image:url('${youtubeThumb(story.videoId)}')" role="img" aria-label="${alt}"></div>`;
  }

  function guestBoxHtml(story) {
    if (!story.guest) return "";
    return `
      <div class="desafio-story__guest">
        <span class="desafio-story__guest-kicker">Invitado</span>
        <p class="desafio-story__guest-name">${story.guest.name}</p>
        ${story.guest.company ? `<p class="desafio-story__guest-line">${story.guest.company}</p>` : ""}
        ${story.guest.area ? `<p class="desafio-story__guest-line">${story.guest.area}</p>` : ""}
      </div>
    `;
  }
      ...
      <section class="article-video">
        <div class="video-cover" id="videoCover" data-embed-src="https://www.youtube.com/embed/${story.videoId}${startParamAutoplay}" data-title="${story.pageTitle}">
          ...
          <button type="button" class="video-cover__play" id="videoCoverBtn" aria-label="Ver entrevista">
            ...
            <span class="video-cover__play-label">Ver entrevista</span>
          </button>
          <span class="video-cover__kicker">Desafío 100 Empresas</span>
        </div>
  ...
  function disclaimerHtml(story) {
    if (!story.disclaimer) return "";
    return `
      <div class="article-disclaimer">
        <h2 class="article-disclaimer__heading">Información importante</h2>
        <p class="article-disclaimer__text">${story.disclaimer}</p>
      </div>
    `;
  }

  function sponsorHtml(story) {
    if (!story.sponsor) return "";
    return `
      <div class="desafio-story__sponsor">
        <span class="desafio-story__sponsor-kicker">Patrocinio</span>
        <p class="desafio-story__sponsor-role">${story.sponsor.role}</p>
        <p class="desafio-story__sponsor-note">${story.sponsor.note}</p>
      </div>
    `;
  }
  ...
  function shareHtml(story) {
    ...
    const encodedTitle = encodeURIComponent(story.pageTitle);
    return `
  ...
  function projectModuleHtml() {
    const info = DESAFIO_PROJECT_INFO;
    return `
      <section class="desafio-story__project">
        <h2 class="section-title">${info.title}</h2>
        <div class="article-body">${info.bodyHtml}</div>
        <div class="desafio-story__participate">
          <p class="desafio-story__participate-title">${info.participateTitle}</p>
          <span class="btn-outline btn-outline--dark desafio-story__participate-btn" aria-disabled="true">${info.participateCta}</span>
          <p class="desafio-story__participate-status">${info.participateStatus}</p>
        </div>
      </section>
    `;
  }
  ...
  function setSEO(story) {
    document.title = story.pageTitle + " — El Podcast del Migrante Magazine";
    document.getElementById("metaDescription").setAttribute("content", story.cardDescription);
    document.getElementById("ogTitle").setAttribute("content", story.pageTitle);
    document.getElementById("ogDescription").setAttribute("content", story.cardDescription);
    document.getElementById("twitterTitle").setAttribute("content", story.pageTitle);
  ...
  function renderStory(story) {
    ...
    root.innerHTML = `
      <span class="tag">Desafío 100 Empresas</span>
      <h1 class="article-title">${story.pageTitle}</h1>
      <p class="article-dek">${story.cardDescription}</p>
      ${guestBoxHtml(story)}
      ${heroMediaHtml(story)}
      <div class="article-body">${story.bodyHtml}</div>
      ${videoHtml(story)}
      ${disclaimerHtml(story)}
      ${sponsorHtml(story)}
      ${shareHtml(story)}
      ${projectModuleHtml()}
    `;
